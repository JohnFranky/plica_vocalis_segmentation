...
def getSeedPoints(pathSnakes, pathPicture, frame_number, offsetShiny, forceLowerhalfPnt = False, forceMiddlePnt = False):
    ...
    if(not nosnake):
        ...
        lines = alternateLines(175, 195, 260, 360)
    ...
    for y in range(6, 506):
        ...
        for i in range(0, 256):
            ...
            if binaryImg[y, x] == 255:
                idxEnd = x
                break
        for x in range(idxBeginn, idxEnd + 1):
            binaryImg[y, x] = 255
        for x in range(0, 256):
            for i in range(1, 5):
                if binaryImg[y-i, x] == 255 and binaryImg[y+i, x] == 255:
                    binaryImg[y,x] = 255
...
def topSideCheck(snake_path, binaryImg, frame_number):
    ...
    if(min_y_binImg < min_y_snakes-offset):
        return True
    """
    offset = 32                                                                                                   #Offset from right picture rand
    for x in range(255-offset, 256):
        for y in range(0, 512):
            if(binaryImg[y,x] == 255 or binaryImg[y,x] == 1.0):
                return True
    
    for y in range(260, 360):
        if(binaryImg[y,126] == 255 or binaryImg[y,126] == 1.0):
                return True
    
    for x in range(125, 225):
        if(binaryImg[247,x] == 255 or binaryImg[247,x] == 1.0):
                return True
        if(binaryImg[383, x] == 255 or binaryImg[383, x] == 1.0):
                return True
    
    for y in range(367, 372):
        if(binaryImg[y,143] == 255 or binaryImg[y,143] == 1.0):
                return True
...
def regionGrow(img,seeds, thresh,p = 1, activateLine = False, activatebottom = False):
    ...
    while(len(seedList)>0):
        ...
        for i in range(8):
            ...
            if(activatebottom and (tmpY >= 212 or tmpY <= 144 or tmpX >= 383)):
                continue
...
if __name__ == "__main__":
    Initials = "DD"
    FrameNumberStart = 1
    FrameNumberEnd = 1 #number of last frame included
    ...
    for k in range (FrameNumberStart, FrameNumberEnd + 1):
        ...
        first_img_path = image_path + FrameNumber + r"DD.png"
        ...
        tuple = getSeedPoints(snake_path, first_img_path, FrameNumberInt, offsetShiny, True)
        ...
        bottomSide = binaryImg.copy()
        tuple = getSeedPoints(snake_path, first_img_path, FrameNumberInt, offsetShiny, forceLowerhalfPnt=False, forceMiddlePnt=True)
        for i in range(1, 12):
            #activateLine = True
            tmpImg = regionGrow(first_image, tuple[0], tuple[1]+i, activateLine = True)
            tmpLength = sum(sum(tmpImg))
            #cv2.imshow('RG',tmpImg)
            #cv2.waitKey(0)
            if(tmpLength - length > 5000 or topSideCheck(snake_path, tmpImg, FrameNumberInt)):
        ...
        cv2.imshow('3',bottomSide)
        cv2.imshow('2',binaryImg)
        topSide_normed = 255 * (topSide - topSide.min()) / (topSide.max() - topSide.min())
        topSide_normed = np.array(topSide_normed, np.int)
        bottomSide_normed = 255 * (bottomSide - bottomSide.min()) / (bottomSide.max() - bottomSide.min())
        bottomSide_normed = np.array(bottomSide_normed, np.int)
        binaryImg_normed = 255 * (binaryImg - binaryImg.min()) / (binaryImg.max() - binaryImg.min())
        binaryImg_normed = np.array(binaryImg_normed, np.int)
        cv2.imwrite(path + r"\\" + r"Top.png", topSide_normed)
        cv2.imwrite(path + r"\\" +r"Bot.png", bottomSide_normed)
        cv2.imwrite(path + r"\\" +r"Mid.png", binaryImg_normed)
        #cv2.waitKey(0)
