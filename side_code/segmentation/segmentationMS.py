...
def topSideCheck(snake_path, binaryImg, frame_number):
    ...
    offset = -8 #MS = -2 #MK = 5                                                                                  #Offset in topSide: How much tolerance should there be between top point glottis and top point vocalis? {MK: 5, MS: 1}
    ...
    if(min_y_binImg < min_y_snakes-offset):
        return True
    offset = 15                                                                                                   #Offset from right picture rand
...
if __name__ == "__main__":
    Initials = "MS"
    FrameNumberStart = 119
    FrameNumberEnd = 119 #number of last frame included
    #FrameNumberInt = int(FrameNumber) 
    offsetShiny = 9 #offset to favor brighter pnts in seed evaluation. Maybe this needs to be changed in regards to different data #MK=5
    if(Initials == "CF"):
