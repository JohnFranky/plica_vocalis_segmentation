...
def getSeedPoints(pathSnakes, pathPicture, frame_number, offsetShiny):
    ...
    if(diff_topToSide > 10):
        """
        for i in range(min_y_id + diff_topToSide + offset, max_y_id - offset):
            linepnts.append(Point(snakes[i][1], snakes[i][0]- pxlOffset))
        
        for i in range(max_y_id + offset, min_y_id - diff_topToSide - offset):
            linepnts.append(Point(snakes[i][1], snakes[i][0] + pxlOffset))
        """
    else:
        """
        aim was here to get shiny pnts on top most side, but failed due to too big off an threshold
        """
    for i in range(min_x_id + offset, max_y_id - offset):
        linepnts.append(Point(snakes[i][1], snakes[i][0]- pxlOffset))
        
    for i in range(max_y_id + offset, max_x_id - offset):
        linepnts.append(Point(snakes[i][1], snakes[i][0] + pxlOffset))

    """
    #Solution 1: HalfCircle
    #linepnts = [Point(max_y[1]+20, max_y[0]),Point(max_y[1], max_y[0]+20),Point(max_y[1], max_y[0]-20)]
    #Get Points on half circle in a radius of 20 pixles below lowest glottis point
    linepnts = []
    addition_y = -20
    addition_x = 0
    halftime = False
    for i in range(0,42):
        linepnts.append(Point(max_y[1]+addition_x, max_y[0]+addition_y))
        if(addition_y == 0):
            halftime = True
        if(halftime):
            addition_x -= 1
...
def topSideCheck(snake_path, binaryImg, frame_number):
    ...
    offset = 1 #MK = 5                                                                                  #Offset in topSide: How much tolerance should there be between top point glottis and top point vocalis? {MK: 5, MS: 1}
    ...
    if(min_y_binImg < min_y_snakes-offset):
        return True
    for y in range(0, 512):
        if(binaryImg[y,255] == 255 or binaryImg[y,255] == 1.0):
            return True
    """
    ...
    while(len(seedList)>0):
        ...
        for i in range(8):
            ...
            if(len(seeds) == 3):
                ...
                length3 = math.sqrt( math.pow((tmpX - origin3.x),2) + math.pow((tmpY - origin3.y),2) )
...
if __name__ == "__main__":
    Initials = "MK"
    FrameNumberStart = 1
    FrameNumberEnd = 471 #number of last frame included
    #FrameNumberInt = int(FrameNumber) 
    offsetShiny = 5 #offset to favor brighter pnts in seed evaluation. Maybe this needs to be changed in regards to different data
    if(Initials == "CF"):
    ...
    for i in range (FrameNumberStart, FrameNumberEnd + 1):
        FrameNumberInt = i
        ...
        #cv2.imshow('Offset: Start',binaryImg)
        if(length > 3500 or (length < 500 and contourCheck(binaryImg)) or fastEndCheck(first_image, tuple, length)):
            i = 5
            length1 = 10000
            while(length1 > 3500):
                ...
                #cv2.imshow('Offset: Higher',binaryImg1)
                #cv2.waitKey(0)
                if(length1 == 1.0 and i > 25):
                ...
                if(contourCheck(binaryImg1) or fastEndCheck(first_image, tuple1, length1)):
                    length1 = 10000

            i = 5
            length2 = 10000
            while(length2 > 3500):
                tuple2 = getSeedPoints(snake_path, first_img_path, FrameNumberInt, offsetShiny - i)
                binaryImg2 = regionGrow(first_image, [tuple2[0]], tuple2[1])
                length2 = sum(sum(binaryImg2))
                i += 4

                #cv2.imshow('Offset: Lower',binaryImg2)
                if(i > 85):
                    length2 = -1
                    break
                if(contourCheck(binaryImg2) or fastEndCheck(first_image, tuple2, length2)):
                    length2 = 10000
            if(length1 == -1):
                if(length2 == -1):
                    print("Error at finding suitable startposition")
                else:
                    length = length2
                    binaryImg = binaryImg2
                    tuple = tuple2
            else:
                if(length2 == -1):
                    length = length1
                    binaryImg = binaryImg1
                    tuple = tuple1
                else:
                    if(length1 > length2):
                        length = length1
        ...
        cv2.imshow('Result of RG',first_image | binaryImg.astype(np.uint8))
        ...
        cv2.imshow("cutGlottis", first_image | binaryImg.astype(np.uint8))
        cv2.waitKey(0)
