...
def getSeedPoints(pathSnakes, pathPicture, frame_number, offsetShiny, forceLowerhalfPnt = False):
    ...
    if(not nosnake):
        ...
        lines = alternateLines(168, 191, 220, 310)
...
def contourCheck(binaryImg):
    ...
    if(lengthInt > 4 * widthInt):
        return True
    if(widthInt > 4 * lengthInt):
        return True
    return False
...
def topSideCheck(snake_path, binaryImg, frame_number):
    ...
    if(min_y_binImg < min_y_snakes-offset):
        return True
    """
    offset = 35                                                                                                   #Offset from right picture rand
    ...
    for y in range(220, 330):
        if(binaryImg[y,145] == 255 or binaryImg[y,145] == 1.0):
                return True
    
    for x in range(140, 215):
        if(binaryImg[185,x] == 255 or binaryImg[185,x] == 1.0):
                return True
        if(binaryImg[355, x] == 255 or binaryImg[355, x] == 1.0):
                return True
...
if __name__ == "__main__":
    Initials = "SS"
    FrameNumberStart = 1
    FrameNumberEnd = 172 #number of last frame included
    #FrameNumberInt = int(FrameNumber) 
    offsetShiny = 4 #offset to favor brighter pnts in seed evaluation. Maybe this needs to be changed in regards to different data #MK=5
    if(Initials == "CF"):
    ...
    for k in range (FrameNumberStart, FrameNumberEnd + 1):
        ...
        cv2.imwrite(os.path.join(path, "Segmentation", "image1.png"), 255*binaryImg)
        ...
        cv2.imwrite(os.path.join(path, "Segmentation", "image2.png"), first_image | binaryImg.astype(np.uint8))
        cv2.waitKey(0)
