...
def getSeedPoints(pathSnakes, pathPicture, frame_number, offsetShiny):
    ...
    if(not nosnake):
        ...
        lines = alternateLines(170, 210, 230, 280)
    ...
    if(not doppelt):
        ...
        if(avgOffset - linepntsGrayLeft[idx1] > 5):
            
            avgOffset = int(linepntsGrayLeft[idx1]) #int(avgLeft) + (linepntsGrayLeft[idx1] - int(avgLeft))/2
            idx1 = find_nearest(linepntsGrayLeft, avgOffset)
            idx2 = find_nearest(linepntsGrayRight, avgOffset)
        if(avgOffset - linepntsGrayRight[idx2] > 5):
            avgOffset = int(linepntsGrayRight[idx2]) #int(avgRight) + (linepntsGrayRight[idx2] - int(avgRight))/2
            idx1 = find_nearest(linepntsGrayLeft, avgOffset)
            idx2 = find_nearest(linepntsGrayRight, avgOffset)
    ...
    for x in linepnts:
        image[x.x, x.y] = 255
    loa = cv2.imread('E:\\Eigene Dateien Jonathan\\studium\\6. Semester\\Bachlor\\HLE_Dataset\\LS\\png\\\\00005LS.png',0) | image.astype(np.uint8)
    cv2.imshow("linepnts", loa)
    cv2.imwrite(os.path.join(r"E:\\Eigene Dateien Jonathan\\studium\\6. Semester\\Bachlor\\HLE_Dataset\\LS", "Segmentation", "image.png"), loa)
    cv2.waitKey(0)
    """
...
def topSideCheck(snake_path, binaryImg, frame_number):
    ...
    for y in range(270, 305):
        if(binaryImg[y,230] == 255 or binaryImg[y,230] == 1.0):
                return True
    
    for x in range(160, 172):
        if(binaryImg[188,x] == 255 or binaryImg[188,x] == 1.0):
                return True
    
    for x in range(170, 205):
        if(binaryImg[350, x] == 255 or binaryImg[350, x] == 1.0):
                return True
    """
...
if __name__ == "__main__":
    Initials = "LS"
    FrameNumberStart = 1
    FrameNumberEnd = 300 #number of last frame included
    #FrameNumberInt = int(FrameNumber) 
    offsetShiny = 1 #offset to favor brighter pnts in seed evaluation. Maybe this needs to be changed in regards to different data #MK=5
    if(Initials == "CF"):
    ...
    for k in range (FrameNumberStart, FrameNumberEnd + 1):
        ...
        if(FrameNumberInt < 10):
            FrameNumber = r"0000"+str(FrameNumberInt) +str("LS")
