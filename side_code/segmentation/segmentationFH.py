...
def getSeedPoints(pathSnakes, pathPicture, frame_number, offsetShiny, forceLowerhalfPnt = False):
    ...
    if(pnts < 30):
        lines = alternateLines(193, 220, 270, 345)
        linepntsLeft = lines[0]
        linepntsRight = lines[1]
        linepntsGrayLeft = []
        linepntsGrayRight = []
...
def topSideCheck(snake_path, binaryImg, frame_number):
    ...
    if(min_y_binImg < min_y_snakes-offset):
        return True
    """
    offset = 10                                                                                                   #Offset from right picture rand
    ...
    for x in range(170, 240):
        if(binaryImg[255,x] == 255 or binaryImg[255,x] == 1.0):
                return True
        if(binaryImg[385, x] == 255 or binaryImg[385, x] == 1.0):
                return True
...
if __name__ == "__main__":
    Initials = "FH"
    FrameNumberStart = 24
    FrameNumberEnd = 24 #number of last frame included
    ...
    for k in range (FrameNumberStart, FrameNumberEnd + 1):
        ...
        #cv2.imshow("cutGlottis", binaryImg)
        #cv2.waitKey(0)
        new_img = binaryImg | first_image
        #cv2.imshow("error", new_img)
        #cv2.waitKey(0)
