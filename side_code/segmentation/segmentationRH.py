...
def getSeedPoints(pathSnakes, pathPicture, frame_number, offsetShiny, forceLowerhalfPnt = False, forcePxlOffset = False):
    ...
    if(not nosnake):
        ...
        lines = alternateLines(200, 220, 290, 350)
        linepntsLeft = lines[0]
        linepntsRight = lines[1]
        linepntsGrayLeft = []
        linepntsGrayRight = []
    #if(forceLowerhalfPnt):
    res = rebuildLines(linepntsLeft, linepntsRight, 325, bot = forceLowerhalfPnt)
    linepntsLeft = res[0]
    linepntsRight = res[1]
...
def correctImage(binImg, topSide):
    leftCorner = 1000
    rightCorner = -1
    topCorner = 1000
    ...
    for x in range(0, 256):
        for y in range(0, 512):
            if((topSide[y,x] == 255 or topSide[y,x] == 1.0)):
...
def topSideCheck(snake_path, binaryImg, frame_number):
    ...
    for y in range(240, 360):
        if(binaryImg[y,162] == 255 or binaryImg[y,162] == 1.0):
                return True
    
    for x in range(165, 245):
        if(binaryImg[240,x] == 255 or binaryImg[240,x] == 1.0):
                return True
    
    for x in range(150, 230):
        if(binaryImg[380, x] == 255 or binaryImg[380, x] == 1.0):
                return True
    """
...
def regionGrow(img,seeds, thresh,p = 1, activateLine = False):
    doubleSeed = False
    if(len(seeds) > 1):
        doubleSeed = True
    ...
    while(len(seedList)>0):
        ...
        for i in range(8):
            ...
            if(activateLine and (tmpX <= 287 or tmpY <= 167)):
                continue
...
if __name__ == "__main__":
    Initials = "RH"
    FrameNumberStart = 102
    FrameNumberEnd = 327 #number of last frame included
    #FrameNumberInt = int(FrameNumber) 
    offsetShiny = -7 #offset to favor brighter pnts in seed evaluation. Maybe this needs to be changed in regards to different data #MK=5
    if(Initials == "CF"):
    ...
    for k in range (FrameNumberStart, FrameNumberEnd + 1):
        ...
        first_img_path = image_path + FrameNumber + r"RH.png"
        ...
        if(doppelt):
            ...
            if(length1 > 3500 or length1 < 10 or (length1 < 500 and contourCheck(binaryImg1)) or fastEndCheck(first_image, tuple, length1) or topSideCheck(snake_path, binaryImg1, FrameNumberInt) or length2 > 3500 or length2 < 10 or (length2 < 500 and contourCheck(binaryImg2)) or topSideCheck(snake_path, binaryImg2, FrameNumberInt) or fastEndCheck(first_image, tuple, length2)):
                ...
                while(length1 > 1850 or length2 > 1850 or length1 > 100*length2 or length2 > 100*length1):
                    ...
                    tuple2 = getSeedPoints(snake_path, first_img_path, FrameNumberInt, offsetShiny + i, False, True)
                    binaryImg1 = regionGrow(first_image, [tuple1[0][0]], tuple1[1])
                    length1 = sum(sum(binaryImg1))
                    binaryImg2 = regionGrow(first_image, [tuple2[0][1]], tuple2[1])
                    length2 = sum(sum(binaryImg2))
        ...
        tuple = getSeedPoints(snake_path, first_img_path, FrameNumberInt, offsetShiny + 16, True)
        ...
        for i in range(1, 35):
            ...
            if(tmpLength - length > 5000 or topSideCheck(snake_path, tmpImg, FrameNumberInt)):
                ...
                for j in range(i, 3*i):
                    tmpImg = regionGrow(first_image, tuple[0], tuple[1]+j)
                    tmpLength = sum(sum(tmpImg))
                    cv2.imshow('jo',tmpImg)
                    #cv2.waitKey(0)
                    length = tmpLength
                    binaryImg = tmpImg
                """
        ...
        cv2.imwrite(path + result_path + r"\\" + FrameNumber + r"RH.png", binaryImg)
