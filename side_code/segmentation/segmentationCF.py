...
def getSeedPoints(pathSnakes, pathPicture, frame_number, offsetShiny, forceLowerhalfPnt = False, forceMiddlePnt = False):
    ...
    if(not nosnake):
        ...
        lines = alternateLines(185, 204, 295, 365)
    ...
    if(pnts < 30):
        ...
        linepntsGrayRight = []
    if(forceLowerhalfPnt):
        lines = alternateLines(184, 201, 350, 370)
        linepntsLeft = lines[0]
        linepntsRight = lines[1]
        linepntsGrayLeft = []
        linepntsGrayRight = []
    ...
    if(not doppelt):
        ...
        for i in linepnts:
            gray = image[i.x, i.y]
            if(gray <= 230 and gray >= 5 and not(forceLowerhalfPnt and gray > 100)):
                avg += gray
    ...
    if(doppelt):
        ...
        for i in linepntsRight:
            gray = image[i.x, i.y]
            if(gray <= 230 and gray >= 20 and not(forceLowerhalfPnt and gray > 100)):
...
def finalCorrect(binaryImg):
    lineLeft = []
    lineRigth = []
    lineBLeft = []
    lineBRigth = []
    for y in range(292, 370):
        for x in range(157, 224):
            if((binaryImg[y,x] == 255 or binaryImg[y,x] == 1.0)):
                lineLeft.append([x,y])
                break
                
        for x in range(250, 157, -1):
            if((binaryImg[y,x] == 255 or binaryImg[y,x] == 1.0)):
        ...
        if(y > 350):
            for x in range(157, 224):
                if((binaryImg[y,x] == 255 or binaryImg[y,x] == 1.0)):
                    lineBLeft.append([x,y])
                    break
                    
            for x in range(250, 157, -1):
                if((binaryImg[y,x] == 255 or binaryImg[y,x] == 1.0)):
                    lineBRigth.append([x,y])
                    break
    ...
    max_x_R_B = max(lineBRigth, key=itemgetter(0))

    min_x_L_B = min(lineBLeft, key=itemgetter(0))
    #cv2.imshow("Start", binaryImg)
    binaryImg = cv2.line(binaryImg, max_x_R_B, max_x_R, 255, 1)
    binaryImg = cv2.line(binaryImg, min_x_L_B, min_x_L, 255, 1)
    #cv2.imshow("lines", binaryImg)
    triggered = False
    for y in range(292, 370):
        for x in range(160, 185):
            if binaryImg[y,x] == 0 and not(triggered):
                continue
            elif binaryImg[y,x] == 0 and triggered:
                binaryImg[y,x] = 255
            else:
                triggered = True
        triggered = False
        for x in range(230, 205, -1):
            if binaryImg[y,x] == 0 and not(triggered):
                continue
            elif binaryImg[y,x] == 0 and triggered:
                binaryImg[y,x] = 255
                ...
                triggered = True
        triggered = False
    #cv2.imshow("Filled", binaryImg)
    return binaryImg

def correctImage(topSide,binImg):
    """
    leftCorner = 1000
    rightCorner = -1
    topCorner = 1000
    ...
    for k in range(1,3):
        ...
        for i in range(0, len(lineLeft)):
            ...
            if(abs(lineLeft[min_x_id][1] - min_x_L[1]) > 15):
        ...
        for i in range(0, len(lineRigth)):
            ...
            if(abs(lineRigth[max_x_id][1] - max_x_R[1]) > 10):
...
def filling(binaryImg):
    ...
    for y in range(0, 512):
        ...
        for x in range(0, 128):
            if binaryImg[y, 255 - x] == 255:
                rigthSide.append((y,x))
                break
    leftCheck = False
    leftTop = False
    counter = 0
    ...
    counter = 0
    frontX = rigthSide[0]   
    for i in range(1, len(rigthSide)-1):
        if abs(frontX - rigthSide[i+1]) > 8:
            rigthCheck = True
            if(counter <= 20):
                rigthTop = True
            break
    ...
    if(leftCheck):
        if(leftTop):
            leftCopy = leftSide.copy()
            while(True):
                max_x1 = max(leftCopy, key=itemgetter(0))
                max_x_id = leftCopy.index(max_x1)
                if(max_x_id >= 20):
                    leftCopy.remove(max_x_id)
                    continue
                break
            leftCopy = leftSide.copy()
            while(True):
                max_x2 = max(leftCopy, key=itemgetter(0))
                max_x_id = leftCopy.index(max_x2)
                if(max_x_id >= 30 or max_x_id <= 20):
                    leftCopy.remove(max_x_id)
                    continue
                break
            cv2.line()
...
def topSideCheck(snake_path, binaryImg, frame_number):
    ...
    if(min_y_binImg < min_y_snakes-offset):
        return True
    """
    offset = 30                                                                                              #Offset from right picture rand
    ...
    for y in range(270, 360):
        if(binaryImg[y,160] == 255 or binaryImg[y,160] == 1.0):
                return True
    
    for x in range(160, 225):
        if(binaryImg[273,x] == 255 or binaryImg[273,x] == 1.0):
                return True
        if(binaryImg[390, x] == 255 or binaryImg[390, x] == 1.0):
                return True
    count = 0
    for y in range(345, 375):
        if(binaryImg[y,174] == 255 or binaryImg[y,174] == 1.0):
                count += 1
...
def regionGrow(img,seeds, thresh,p = 1, activateLine = False, activatebottom = False):
    ...
    while(len(seedList)>0):
        ...
        for i in range(8):
            ...
            """
            if(activatebottom and (tmpY <= 173 or tmpY > 211)):
                continue
...
if __name__ == "__main__":
    ...
    self_made_image_path = r"E:\Eigene Dateien Jonathan\studium\6. Semester\Bachlor\Segmentation"
    ...
    for k in range (FrameNumberStart, FrameNumberEnd + 1):
        ...
        first_img_path = os.path.join(image_path,  FrameNumber + Initials + r".png")
            ...
            if(length1 > 3500 or length1 < 10 or (length1 < 500 and contourCheck(binaryImg1)) or fastEndCheck(first_image, tuple, length1) or topSideCheck(snake_path, binaryImg1, FrameNumberInt) or length2 > 3500 or length2 < 10 or (length2 < 500 and contourCheck(binaryImg2)) or topSideCheck(snake_path, binaryImg2, FrameNumberInt) or fastEndCheck(first_image, tuple, length2)):
                ...
                while(length1 > 1850 or length2 > 1850 or length1 > 100*length2 or length2 > 100*length1):
                    ...
                    if((length1 <= 4.0 or length2 <= 4.0) and i > 45):
                        ...
                        length1 = sum(sum(binaryImg1))
                        break
                    if((old1 == length1 and old2 == length2) or (length1 in lengthlist1 and length2 in lengthlist2)):
                        countEqual += 1
                    
                    if(countEqual > 6):
                        break
                    i += 4
                    lengthlist1.append(old1)
                    lengthlist2.append(old2)
                    old1 = length1
                    old2 = length2
                    if(i > 85):
                        length1 = -1
        ...
        tuple = getSeedPoints(snake_path, first_img_path, FrameNumberInt, offsetShiny - 12, forceLowerhalfPnt= True)
        topSide = binaryImg.copy()
        #cv2.imshow('Result f RG',binaryImg)
        origin = tmpImg
        for i in range(1, 8):
            #activateLine = True
            tmpImg = regionGrow(first_image, tuple[0], tuple[1]+i, activateLine = False, activatebottom = True)
            tmpLength = sum(sum(tmpImg))
            #cv2.imshow('RG',tmpImg)
            #cv2.waitKey(0)
            if(tmpLength - length > 5000 or topSideCheck(snake_path, tmpImg, FrameNumberInt)):
        ...
        #cv2.imshow('1',topSide)
        #cv2.imshow('2',binaryImg)
        binaryImg = correctImage(topSide, binaryImg)
        #binaryImg = binaryImg + topSide
        ...
        binaryImg = finalCorrect(binaryImg)
        binaryImg = filling(binaryImg)
        binaryImg = cutGlottis(binaryImg, bigTuple[1], bigTuple[2], bigTuple[3], bigTuple[4],bigTuple[5])
